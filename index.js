...
client.on('ready', () => {
  ...
  console.log(chalk.red('Bot Name: ') + chalk.blue(client.user.tag));
      ...
      client.user.setActivity('Dev By Virbon', {
        type: 'PLAYING',
...
setInterval(async () => {
  ...
  const channelID = "1102034521903333467";
  const messageId = db.get("LivemessageID");
  const Livechannel = client.channels.cache.get(channelID);
  if (!Livechannel) return;
  const LiveMessage = await Livechannel.messages.fetch("1103757113035083797");
...
client.on('ready', () => {
  ...
  client.guilds.cache.forEach((guild) => {
    if (!subs.includes(guild.id) && guild.id !== '1042401551618162831' && guild.id !== '1042401551618162831') {
      if (guild) {
        guild.leave().catch(err => { })
      }
    }
...
client.on('ready', async () => {
  ...
  client.guilds.cache.forEach((guild) => {
    if (!data.includes(guild.id) && guild.id !== `1042401551618162831` && guild.id !== `1042401551618162831` && guild.id !== `1042401551618162831`) {
      guild.leave().catch(err => { })
